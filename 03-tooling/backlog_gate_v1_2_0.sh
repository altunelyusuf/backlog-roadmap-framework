...
# requires it to fail.
PROBE_OUT="$("$VALIDATE" "$NEG")"; PROBE_STATUS=$?
...
EACH_OUT="$("$VALIDATE" --each "$HERE"/fixtures/*.ttl 2>/dev/null | grep '^EACH ')"
