...
def _negative_fixtures(fixtures_dir):
    ...
    for f in sorted(glob.glob(os.path.join(fixtures_dir, "*.ttl"))):
        ...
        try:
            ...
            continue
        # v1.0.1: polarity is read from ANY subject (see backlog_validate v1.6.0);
        # the profile-only reader saw 5 of the 26 negative fixtures.
        for _s, pol in g.subject_objects(B.hasExpectedPolarity):
            if str(pol).endswith("Polarity_Negative"):  # audit-exempt: iri-suffix-match
                out.append(f)
                break
    return out
